package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      private const const_1706:int = 0;
      
      private var var_123:AvatarRenderManager;
      
      private const const_367:int = 2;
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               var_487.add(_loc4_,new PetData(_loc3_));
            }
         }
      }
      ...
      public function assetsReady(param1:int, param2:IPetDataListener = null) : Boolean
      {
         ...
         {
            _loc4_ = _assets.loadFromFile(new URLRequest(_loc5_));
      ...
      {
         var _loc4_:PetData = var_487.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      private function onLoaderComplete(param1:Event) : void
      {
         ...
         var _loc3_:int = var_249.indexOf(_loc2_);
         if(_loc3_ >= 0)
         {
      ...
      private function notifyListeners() : void
      {
         ...
         while(false)
         {
            _loc1_ = var_689.pop() as IPetDataListener;
            if(_loc1_ != null)
            {
               _loc1_.petDataReady();
            }
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = var_487.getKeys();
         for each(key in speciesKeys)
         {
            petData = var_487.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.RoomEntryInfoMessageEvent.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      {
         var _loc3_:IPetData = var_487.getValue(param1) as IPetData;
