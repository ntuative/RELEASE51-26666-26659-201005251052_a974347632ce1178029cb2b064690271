package com.sulake.core.window.utils
{
   ...
   public class FocusManager extends ChildEntityArrayReader implements IWindowFocusManagerService
   {
      ...
      public function FocusManager(param1:DisplayObject)
      {
         ...
         var_193.addEventListener(Event.ACTIVATE,onActivateEvent);
         var_193.addEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
         var_193.addEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
         var_193.addEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
         super();
      }
      ...
      public function dispose() : void
      {
         if(!_disposed)
         {
            var_193.removeEventListener(Event.ACTIVATE,onActivateEvent);
            var_193.removeEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
            var_193.removeEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
            var_193.removeEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
            var_193 = null;
            _disposed = true;
            var_32 = null;
         }
